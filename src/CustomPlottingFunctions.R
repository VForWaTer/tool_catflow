...
ScatterHist = function(frame, xvar, yvar, title, hill, ...,
  ...
  plot_center = ggplot2::ggplot(NULL, ggplot2::aes_string(x=xvar,y=yvar)) +
    ggplot2::geom_point(data=frame,color=point_color, alpha=point_alpha) +
    ggplot2::theme(plot.margin = grid::unit(c(0, 0, 0, 0), "lines"))+
    ggplot2::geom_line(data=hill$short_rep_hill, aes(x = short_dist, y = short_elev)
                       ,size=2,color='violet')
  if(!is.null(gSmooth)) {
